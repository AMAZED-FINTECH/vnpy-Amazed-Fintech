...
- 备注:策略启动的时候,最后分钟数据的获取一定要在03秒之后,也就是gateway自动推送新的一分钟数据的时候

## 20190702
- 1 版本可视化已调试好,今天出现一个"或有"bug,
event传到可视化那边会将类都删除,变成str,也就是Exchange->exchange.value,
已用try Except调试好Enum的set content
- 2 调试一段时间,让策略运行一段时间

## 20190703
- 1 实盘账户测试中,1张为单位
- 2 进行本地buy sell short cover 本地停止单的测试
- 3 对engine里面的check stop order 进行调整,如果没有涨停价也没有5档价格时怎么办
