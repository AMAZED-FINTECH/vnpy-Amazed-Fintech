...
class DoubleEmaStrategy(CtaTemplate):
    ...
    def on_5min_bar(self, bar: BarData):
        ...
        if cross_over:
            if self.pos == 0:
                # 买 开
                self.buy(round(bar.close_price*1.01, 3), 1)
            elif self.pos < 0:
                # 平 空
                self.cover(round(bar.close_price*1.01, 3), 1)
                # 买 开
                self.buy(round(bar.close_price*1.01, 3), 1)
            ...
            if self.pos == 0:
                # 空 开
                self.short(round(bar.close_price*0.99, 3), 1)
            elif self.pos > 0:
                # 平 多
                self.sell(round(bar.close_price*0.99, 3), 1)
                # 空 开
                self.short(round(bar.close_price*0.99, 3), 1)
        self.put_event()
